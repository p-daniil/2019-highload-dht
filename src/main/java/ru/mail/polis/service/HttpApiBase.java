...
public abstract class HttpApiBase extends HttpServer implements Service {
     ...
     */
    public HttpApiBase(final int port, final InternalDAO dao) throws IOException {
    ...
    protected Response get(final ByteBuffer key) throws IOException {
        ...
        try {
            value = dao.getValue(key).getData();
    ...
    @NotNull
    protected Response getV(final ByteBuffer key) throws IOException {
        final Value value;
        try {
